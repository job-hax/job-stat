...
class Parse():
    ...
    def __init__(self, verbose=False):
        ...
        fields = ["name", "address", "email", "phone", "mobile", "telephone", "residence status",
                  "experience", "degree","cainstitute","cayear","caline","b.cominstitute","b.comyear",
                  "b.comline","icwainstitue","icwayear","icwaline","m.cominstitute","m.comyear",
                  "m.comline","mbainstitute","mbayear","mbaline"]

        # Glob module matches certain patterns
        doc_files = glob.glob("sample/*.doc")
    ...
    def preprocess(self, document):
        ...
        try:
            ...
            lines = [el.strip() for el in document.decode().split("\n") if len(el) > 0]  # Splitting on the basis of newlines
    ...
    def getPhone(self, inputString, infoDict, debug=False):
        ...
        try:
            ...
            # hyphen has to be escaped inside the character class if it's not an indication of range
        ...
        try:           
            ...
            if index:#only finds for Ca rank and CA year if it finds the word Ca in the document
                ...
                for indextocheck in index:#checks all nearby lines where it founds the degree word.ex-'CA'
                    checklines = [indextocheck, indextocheck + 1] if indextocheck + 1 < len(self.lines) else [indextocheck]
                    for i in checklines: #checks the line with the keyword and just the next line to it
                        try:
                            try:
